        ...
        <div class="grid-table-container" ref="container">
            <table class="grid-table" ref="table" :class="{ filters: enableFilters }">
                ...
                <tr class="headers-row" :style="{ 'grid-template-columns': gridTemplateColumns }">
                    <th v-for="(column,index) in columnDefs" :key="index"
                        :class="{ sortable: column.sortable, sorting: column.field === sortByColumn, descending: sortByDesc }"
                        @click="sort(column)">
                        <span class="header-content">{{ column.headerName }}</span>
                        <span class="resize-handle" @mousedown="initResize(column, $event)" @click.stop></span>
                    </th>
                </tr>
                ...
                <div :style=" { 'min-height': `${rowDataPage.length * itemHeight}px` }">
                    <tr class="gridrow" v-for="(row, rowIndex) in visibleRows" :key="row[rowDataKey]"
    ...
    export default {
        ...
        methods: {
            ...
            async selectCell(rowIndex, colIndex, $event) {
                //triggered when clicking on a cell
                if (changePending) {
                    return;
                }
            ...
            focus() {
                //TODO: remove selStart, change to single cell, select entire row on edit, allow tabbing through rows.
                const [rowIndex, colIndex] = this.selStart;
                const body = this.$refs.body;
            ...
            initResize(column, $event) {
                //todo: prevent resizing bigger than the bounding box (no horizontal scrolling)
                //todo: moving the right-most resizer should shrink the right-most row instead.
                initResize($event.target.parentNode, $event, width => {
                    column.size = Math.max(30, width) + 'px';
                    this.setGridColumnTemplate()
                }, () => {
                    localStorage.setItem(`${this.id}_columns`, JSON.stringify(this.columnDefsFiltered.map(({field, size}) => ({
                        field,
                        size
                    }))))
                })
